...
# Train
import matplotlib
matplotlib.use('Agg')  # Use the 'Agg' backend for rendering without a graphical display
...
for e in range(1, 100+1):
    ...
    with torch.no_grad():
        ...
        x_0 = x_0.permute(0, 2, 3, 1).clamp(0, 1).detach().cpu().numpy()
        for i in range(5):
            plt.imshow(x_0[i])
            plt.savefig('output_image_{}.png'.format(i))
    torch.save({
            'epoch': e,
            'model_state_dict': model.state_dict(),
            'optimizer_state_dict': optim.state_dict(),
            'loss': loss,
            }, model_save_dir+"epoch{0:0>3}_model.tar".format(e))